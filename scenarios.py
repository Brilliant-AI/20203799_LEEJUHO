...
class VirtualScenario(object):
    ...
    def submit(self):
        ...
        #print(len(detection_ranks), detection_ranks)
        detected_failures = len(detection_ranks)

        ## total number od failures present
        #print(len(self.solutions.values()), self.solutions.values())
        tmp_values = list(self.solutions.values())
        #print(len(tmp_values), tmp_values)
        total_failure_count = sum(self.solutions.values())
        if len(tmp_values) < 5:
            window_failure_count = 0
        else:
            window_failure_count = sum(tmp_values[-window_size:])
        #print(window_failure_count)
        ...
        if total_failure_count > 0:
            ...
            if window_failure_count == 0:
                ...
                APHFW = 1 - sum(detection_ranks) / (window_failure_count * window_size) + 1 / (2 * window_size)
            ...
            ttf = 0
            APHFW = 0
            #처음에는 napfd 1로 설정 되어 있었음.
            napfd = 0
            recall = 1
            avg_precision = 1

        #print(APHFW)
        return [detected_failures, undetected_failures, ttf, napfd, recall*100, avg_precision, detection_ranks, APHFW]
